...
PS：JVM 会在编译时期将 boolean 类型的数据转换为 int，使用 1 来表示 true，0 表示 false，此时占 4 个字节。JVM 支持 boolean 数组，但是是通过读写 byte 数组来实现的，此时占 1 个字节。
在《Java虚拟机规范》给出了单个boolean占4个字节，和boolean数组1个字节的定义，具体还要看虚拟机实现是否按照规范来，所以1个字节、4个字节都是有可能的。
...
java 8 针对接口新增了 default 方法和接口静态方法。
